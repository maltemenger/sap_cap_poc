...
export class AuthenticationDataService implements IAuthenticationDataService {
  ...
  async get_authentication_data_for_businesspartner(kundennummer: string): Promise<CustomerAuthenticationData> {
    ...
    const response = await authProviderService.send({ event: 'get_auth_infos', entity: 'authprovider', data: { kundennummer} });
    console.log('REPSONSE ', response)

   return  await authProviderService.send({ event: 'get_auth_infos', data: {kundennummer} });
   // console.log('response ');
   // console.log(response)
   // return  await authProviderService.post("get_auth_infos", { kundennummer });
